...
export default function AdminProductsPage() {
  ...
  async function save() {
    ...
    try {
      if (editing.id) {
        await apiFetch(`/admin/products/${editing.id}`, { method: 'PATCH', body: JSON.stringify(editing) });
      } else {
        await apiFetch('/admin/products', { method: 'POST', body: JSON.stringify(editing) });
      }
      setEditing(null);
      load();
    } finally {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => setEditing({ ...emptyForm })}><Plus className="h-4 w-4" /> Tambah Produk</Button>
      </div>

      {editing && (
        <Card className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-semibold">{editing.id ? 'Ubah' : 'Baru'} Produk</h2>
              ...
              <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${item.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                    {(item as Product & { status?: string }).status || 'active'}
                  </span>
                </td>
