...
| BF-018 | Homepage services | Section "Apa yang Kami Tawarkan" always showed 6 hardcoded default cards, not data from `/admin/services` | Homepage SSR used raw `NEXT_PUBLIC_API_URL` fetch (same class of bug as BF-016). Switched to `fetchPublicApiList('/services')` with production API resolver + internal fallback. Only **active** services appear; fallback defaults used only when API returns empty. Removed per-card `Tech:` line from `HomeServices`. |

**Files (BF-017):** `frontend/src/app/(public)/products/[slug]/page.tsx`, `frontend/src/components/interactive/ROICalculator.tsx`

**Files (BF-018):** `frontend/src/app/(public)/page.tsx`, `frontend/src/app/(public)/services/page.tsx`, `frontend/src/lib/server-api.ts`, `frontend/src/components/homepage/HomeServices.tsx`, `frontend/src/lib/homepage-content.ts`, `frontend/src/app/admin/services/page.tsx`
