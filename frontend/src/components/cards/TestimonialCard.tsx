...
export function TestimonialCard({ testimonial: t, featured = false }: TestimonialCardProps) {
  return (
    <Card className={featured ? 'border-blue-200 bg-blue-50/30' : undefined}>
      {t.title && (
        <div className="text-xs font-medium text-blue-600 mb-2 uppercase tracking-wide">{t.title}</div>
      )}
      ...
      <blockquote className={`text-slate-600 italic leading-relaxed ${featured ? 'text-lg' : 'text-sm'}`}>
        &ldquo;{t.quote}&rdquo;
      </blockquote>
